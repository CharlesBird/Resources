...
from pythonSort import pythonSort, pythonSorted
...
if __name__ == '__main__':
    ...
    #         for name in ['bubbleSort', 'selectionSort', 'insertionSort', 'shellSort', 'mergeSort', 'quickSort', 'heapSort', 'countingSort', 'bucketSort', 'radixSort', 'pythonSort', 'pythonSorted']:
    #             res = [main(name, i, j), main(name, i, j), main(name, i, j)]
    #             print('排序方式{}, 数字范围{}, 排序长度{}, 三次耗时: {}, 平均耗时: {}'.format(name, i, j, res, np.mean(res)))

    for name in ['bubbleSort', 'selectionSort', 'insertionSort', 'shellSort', 'mergeSort', 'quickSort', 'heapSort',
                 'countingSort', 'bucketSort', 'radixSort', 'pythonSort', 'pythonSorted']:
        for i in [100, 1000, 10000, 100000]:
            for j in [100, 1000, 10000]:
                # print('range{}_{}'.format(i, j))
                res = [main(name, i, j), main(name, i, j), main(name, i, j)]
